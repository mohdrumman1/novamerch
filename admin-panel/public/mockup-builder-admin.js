...
// The product mockup PNGs live on the customer-facing site under
// /mockup-builder/.... When this script runs inside the admin-panel (which
// has basePath /admin and no copy of the PNGs), the host page sets
// window.__MOCKUP_ASSET_BASE to the customer-site origin so we can build
// fully-qualified URLs. When running inside the customer site directly,
// __MOCKUP_ASSET_BASE is '' and the path stays relative.
function _assetBase() {
    return (typeof window !== 'undefined' && window.__MOCKUP_ASSET_BASE) || '';
}
function _withBase(path) {
    if (!path) return path;
    if (/^https?:/i.test(path)) return path;
    if (path.startsWith('/')) return _assetBase() + path;
    return path;
}
...
function getImageSrc() {
    ...
    if (S.style && S.colour && p.styleColourImages && p.styleColourImages[S.style] && p.styleColourImages[S.style][S.colour])
        raw = p.styleColourImages[S.style][S.colour];
    else if (S.style && p.styleImages && p.styleImages[S.style]) raw = p.styleImages[S.style];
    else if (S.colour && p.colourImages && p.colourImages[S.colour]) raw = p.colourImages[S.colour];
    else raw = p.image.startsWith('.') || p.image.startsWith('/') ? p.image : 'mockups/' + p.image;
    return _withBase(raw);
}
...
function getBackImageSrc() {
    ...
    if (S.colour && p.backColourImages && p.backColourImages[S.colour]) return _withBase(p.backColourImages[S.colour]);
    return _withBase(p.backImage);
}
...
function toggleQuotePanel() {
    ...
    btn.textContent = quotePanelOpen ? 'Quote ▲' : 'Quote ▼';
}
...
function renderQuotePanel() {
    ...
    toggleBtn.textContent = quotePanelOpen ? 'Quote ▲' : 'Quote ▼';
    panel.classList.toggle('collapsed', !quotePanelOpen);
...
// window.__novamerchMockupAdminLoaded, but we additionally gate the splitter
